...
CodexBridge 是一个将 **桌面 Codex 引擎** 接入 **微信** 的桥接层。  
微信端只负责收发消息，所有计算都在桌面 Codex 上完成。

## 核心架构

```
微信 App → iLink Bot → CodexBridge → Codex CLI (桌面)
              ↑                          ↓
         消息收发                  模型推理 / 技能 / 插件
              ↑                          ↓
...
```bash
git clone git@github.com:chengshuaimao9-design/CodexBridge.git
cd CodexBridge
...
https://github.com/chengshuaimao9-design/CodexBridge
