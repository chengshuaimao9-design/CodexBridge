...
import { useCodexWorkspace } from '@/components/codex-workspace-context';
...
export function CodexSessionsEmptyState() {
  const { sidebarOpen, threads, toggleSidebar } = useCodexWorkspace();
    ...
    <main className="workspace-main workspace-main--empty">
      ...
      <section className="workspace-empty-state">
        <div className="workspace-empty-state-inner">
          ...
          <p className="workspace-subtle">左侧已准备好 {threads.length} 条会话</p>
        </div>
      </section>
    </main>
  );
...
export function CodexThreadPane({
  ...
  const activeThread = threads.find((entry) => entry.threadId === threadId) ?? null;
  ...
  useEffect(() => {
    ...
    setSendingReply(false);
    closeReplyStream();
  }, [initialThreadHasMore, initialThreadMessages, threadId]);
  ...
  async function handleReplySubmit(event: FormEvent<HTMLFormElement>) {
    ...
    setThreadMessages((current) => [
      ...
      ...buildStreamAssistantMessages(assistantDraftId, timestamp, '', '', 'queued'),
    ]);
    setComposerText('');
