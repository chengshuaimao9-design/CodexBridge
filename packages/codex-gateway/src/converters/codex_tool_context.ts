...
export function responsesToolsToChatTools(
  ...
  for (const tool of tools) {
    ...
    switch (type) {
      ...
      case 'local_shell':
      case 'computer_use': {
        if (type === 'web_search' && options.builtinToolConverter) {
...
function detectCodexCustomToolKind(tool: JsonRecord, name: string): CodexCustomToolKind {
  ...
  if (['web_search', 'local_shell', 'computer_use'].includes(stringValue(tool.type))) {
    return 'built_in';
  }
  return 'raw';
}
...
function normalizeChatToolParameters(parameters: unknown): JsonRecord {
  ...
  return cloneJson(parameters as JsonRecord);
}
