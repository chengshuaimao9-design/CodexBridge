...
Semantic `file_search` is now available through `createCodexProviderRelayInMemoryVectorFileSearchSource()` and the generic `CodexProviderRelayEmbeddingProvider` interface. `createCodexProviderRelayEmbeddingsApiProvider()` targets OpenAI-compatible embeddings APIs with host-provided endpoint, model, headers, and API key. The default endpoint/model currently points at OpenRouter's Qwen embedding API only as a convenient starter configuration, and `createCodexProviderRelayOpenRouterEmbeddingProvider()` remains a thin convenience wrapper over the generic provider. This gives CodexBridge, CodexNext, and future app-servers a real vector-search path today while leaving Qdrant, LanceDB, pgvector, and remote-doc adapters as separate source implementations behind the same contract.
