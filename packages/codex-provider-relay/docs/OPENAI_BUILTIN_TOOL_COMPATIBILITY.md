...
This document tracks how `@codexbridge/codex-provider-relay` maps OpenAI Responses built-in tools to package-owned behavior.
...
2. Relay-emulated tools must have registered executors.
   Example: `hostedToolExecutors.file_search = createCodexProviderRelayFileSearchExecutor(...)`.
...
- Done: added `createCodexProviderRelayToolSearchExecutor()`.
