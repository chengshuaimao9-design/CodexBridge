...
- The SDK includes a generic embedding provider contract, a generic OpenAI-compatible embeddings API provider, a thin OpenRouter/Qwen default wrapper, and an in-memory vector `file_search` source. This is the first semantic-search source and validates the vector adapter surface before adding Qdrant, LanceDB, pgvector, or remote-doc backends.
